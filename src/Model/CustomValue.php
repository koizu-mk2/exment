    ...
    public function getAuthoritable($related_type)
    {
        ...
        if (!is_null($reqSessions)) {
            return $reqSessions->filter(function ($value) use ($related_type) {
                ...
                if ($related_type == SystemTableName::USER) {
                    ...
                    return in_array($value['authoritable_target_id'], \Exment::user()->getOrgIdsForPermission($enum));
                }
            });
        }
        ...
        if ($related_type == SystemTableName::USER) {
            ...
            $query = $this
                ...
                ->whereIn('authoritable_target_id', \Exment::user()->getOrgIdsForPermission($enum));
        } else {
            throw new \Exception;
        }
